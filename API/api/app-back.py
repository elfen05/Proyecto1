...
###Mostrar las ciudades - se usa en boton "ciudades"
...
@app.route("/pais/<id1>/ciudad/<id2>/", methods=["DELETE"])
def borrar_pais_ciudad_porID(id1,id2):
   try :
    cur.execute("DELETE FROM ciudad WHERE pais=%s AND id=%s;", (id1,id2))
    cnx.commit()
    return jsonify({}), status.HTTP_204_NO_CONTENT
  except :
    cnx.rollback()
...
@app.route("/pais/<string:id>/ciudad/<string:id2>/",methods=['GET'])
def mostrar_pais_ciudad_porID(id,id2):
  cur.execute("SELECT * FROM ciudad WHERE pais=%s AND id=%s;", (id,id2))
  listaciudadespais=[]
  for (id, nombre, pais, capital) in cur.fetchall():
    listaciudadespais.append({"id": id, "nombre": nombre, "pais": pais, "capital": capital})
  return jsonify(listaciudadespais), status.HTTP_200_OK 
